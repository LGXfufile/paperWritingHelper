...
def is_hangzhou_westlake_related(comment):
    ...
    try:
        ...
        print("\n程序被用户中断！正在尝试正常退出...")
        raise  # 重新抛出异常以便上层可以继续捕获并处理
    except Exception as e:
        print(f"❌ DeepSeek 处理失败，跳过此条评论: {str(e)}")
        return False
...
def filter_comments(input_file, output_folder, exclusion_file=None):
    ...
    try:
        for index, comment in enumerate(comments):
            ...
            # 判断是否达到设定行数
            if current_line_count % SAVE_EVERY_N_LINES == 0:
                ...
                print(f"✅ 过滤后数据已达到 {SAVE_EVERY_N_LINES}行，处理结果已保存至: {output_path}")
        ...
        if len(filtered_comments) > 0:
            output_filename = f"{base_filename}_part{file_counter}.xlsx"
            output_path = os.path.join(output_folder, output_filename)

            result_df = pd.DataFrame(filtered_comments, columns=["Cleaned Comments"])
            result_df.to_excel(output_path, index=False)

            print(f"✅ 最后的{len(filtered_comments)}条评论已保存至: {output_path}")
    except KeyboardInterrupt:
        print("\n程序被用户中断！正在尝试正常退出...")
        raise  # 重新抛出异常以便上层可以继续捕获并处理
    except Exception as e:
        print(f"❌ 文件处理过程中出现错误: {str(e)}")
...
def main():
    ...
    for filename in xlsx_files:
        ...
        try:
            print(f"\n📦 正在处理文件: {filename}")
            filter_comments(input_path, output_folder, exclude_path)
            success_files += 1
...
if __name__ == '__main__':
    try:
        print(f"✅ 程序将过滤后数据行数超过{SAVE_EVERY_N_LINES}行，就生成一个新的文件，并保存到指定的输出目录下.")
        main()
    except KeyboardInterrupt:
        print("\n程序被用户中断！正在尝试正常退出...")
