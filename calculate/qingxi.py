...
def is_hangzhou_westlake_related(comment):
    ...
    #         {"role": "user","content": f"根据提供的小红书上面游客对于杭州西湖的旅游评论，判断是否是和西湖相关的内容。如果是相关的，请返回 'true'；如果不相关，请返回 'false'. 以下是评论内容: {comment}"}
    #     ],
    #     stream=False
    # )
    ...
    #     messages=[
    #         {"role": "system",
    #          "content": "你是一位乐于助人的助手，擅长理解自然语言处理，并能根据语义分析内容是否与特定主题相关联。"},
    #         {"role": "user",
    #          "content": f"请分析以下从小红书收集的用户评论，判断这些评论是否涉及杭州西湖的旅游体验或相关信息。判断"
    #                     f"时，请考虑评论中可能隐含的与西湖相关的语义内容，比如提及游览景点、享受自然风光、品尝当地美"
    #                     f"食等，即使没有直接提到“杭州”或“西湖”，只要可以合理推断出该评论与西湖旅游有关，请返回 'true'。如果评论明显"
    #                     f"与西湖无关，请返回 'false',不要给出输出其他任何的解释。以下是评论内容: {comment}"}],
    #     stream=False
    # )
